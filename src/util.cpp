...
#else
    const char* pszModule = "stardrillcoin";
#endif
    if (pex)
...
#ifdef WIN32
    // Windows
    return MyGetSpecialFolderPath(CSIDL_APPDATA, true) / "Stardrillcoin";
#else
    fs::path pathRet;
...
boost::filesystem::path GetConfigFile()
{
    ...
    fs::path pathConfigFile(GetArg("-conf", "stardrillcoin.conf"));
...
boost::filesystem::path GetPidFile()
{
    ...
    fs::path pathPidFile(GetArg("-pid", "stardrillcoin.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct.  If your clock is wrong Stardrillcoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    ThreadSafeMessageBox(strMessage+" ", string("Stardrillcoin"), wxOK | wxICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
...
{
    return GetAutostartDir() / "stardrillcoin.desktop";
}
