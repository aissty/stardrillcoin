...
// Global state
//
static const uint256 hashGenesisBlockOfficial("0x000008aa74d7bd6b702c479bfdbda1c0e26833cd0967b1f6984e3689209aa0ea");
static const uint256 hashGenesisBlockTestNet ("0x0000009a72ca2c7e6003bd1e7c26f4241cb7719a24f30a974e4fccf6810b8b39");
...
const string strMessageMagic = "Stardrillcoin Signed Message:\n";
...
void static SyncWithWallets(const CTransaction& tx, const CBlock* pblock = NULL, bool fUpdate = false, bool fConnect = true)
{
    if (!fConnect)
    {
        // Stardrillcoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // Stardrillcoin: enforce minimum output amount
...
{
    return IPO_SHARES / PROOF_OF_WORK_BLOCKS; //this will only be used to create initial SDC
}

// stardrillcoin: minter's coin stake is rewarded based on coin age spent (coin-days)
...
bool CBlock::AcceptBlock()
{
    ...
    // Stardrillcoin: check switch from proof of work to proof of stake
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (pblock->IsProofOfStake())
    {
        ...
        if (!CheckProofOfStake(pblock->vtx[1], pblock->nBits, hashProofOfStake))
        {
            ...
            // stardrillcoin: ask for missing blocks
            if (pfrom)
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "Stardrillcoin", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "Stardrillcoin", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "make by qq 118047098 just for test like this sample connect me^O^!";
        unsigned int nTimeGenesis=1521865946;
        unsigned int nNonceGenesis=1087349;

        if (fTestNet)
        {
            pszTimestamp="make by qq 118047098 just for test like this sample connect me^O^?";
            nTimeGenesis=1521865942;
            nNonceGenesis=2071134;
        }
        ...
        printf("Stardrillcoin Genesis Block Found:\n");
        printf("genesis hash=%s\n", block.GetHash().ToString().c_str());
        printf("merkle root=%s\n", block.hashMerkleRoot.ToString().c_str());
        block.print();
     
        printf("End Stardrillcoin Genesis Block\n");
        ...
        printf("%s\n", block.hashMerkleRoot.ToString().c_str());
        if (!fTestNet)
            assert(block.hashMerkleRoot == uint256("0xd446d6085c61baf85a2a298ad71b2bf2433a9c1e3e48caa0842e8de1d07af7ba"));
        else
            assert(block.hashMerkleRoot == uint256("0x6d024c5a0eac62d237225d62ab8f2e190da99cb9bee4509084216422b8d44c54"));
...
string GetWarnings(string strFor)
{
    ...
    {
        nPriority = 100;
        strStatusBar = "WARNING: Checkpoint is too old. Please wait for the block chain to download, or notify developers of the issue (https://github.com/Stardrillcoin/Stardrillcoin/issues).";
    }
    ...
    {
        nPriority = 3000;
        strStatusBar = strRPC = "WARNING: Invalid checkpoint found! Displayed transactions may not be correct! You may need to upgrade, or notify developers of the issue (https://github.com/Stardrillcoin/Stardrillcoin/issues).";
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (!fAlreadyHave)
                pfrom->AskFor(inv, IsInitialBlockDownload()); // stardrillcoin: immediate retry during initial download
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // stardrillcoin: send latest block
                        vector<CInv> vInv;
