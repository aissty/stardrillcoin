...
{
    // Wait for IRC to get it first - disabled with stardrillcoin
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Stardrillcoin " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
// testnet dns seed begins with 't', all else are stardrillcoin dns seeds.
static const char *strDNSSeed[][2] = {
    // NOTE: Only testnet nodes are included in this build. If you are using this
    // for a real Stardrillcoin offering, you will want to remove the "t" that
    // appears before the node name and IP
    {"Stardrillcoin NODE_2_EU-Ireland", "116.196.66.201"},
    {"Stardrillcoin NODE_3_SE-A_Singapore", "45.77.233.77"},
    {"Stardrillcoin NODE_3_SE-A_Singapore", "207.148.85.220"},
...
unsigned int pnSeed[] =
{
    0xc942c474,0xdc5594cf,0x4de94d2d
};
...
bool BindListenPort(string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, sizeof(sockaddr)) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to port %d on this computer.  Stardrillcoin is probably already running."), ntohs(sockaddr.sin_port));
        else
...
void StartNode(void* parg)
{
    ...
    // IRC disabled with stardrillcoin
