    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    printf("WalletUpdateSpent found spent unit %sSDC %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                if (nChange > 0)
                {
                    ...
                    if (!GetBoolArg("-avatar", true)) // peercoin: not avatar mode; stardrillcoin: avatar mode enabled by default to avoid change being sent to hidden address
                    {
...
string CWallet::SendMoney(CScript scriptPubKey, int64 nValue, CWalletTx& wtxNew, bool fAskFee)
{
    ...
    if (!CommitTransaction(wtxNew, reservekey))
        return _("Error: The transaction was rejected.  This might happen if some of the SDC in your portfolio were already spent, such as if you used a copy of wallet.dat and SDC were spent in the copy but not marked as spent here.");
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                printf("FixSpentCoins found lost unit %sSDC %s[%d], %s\n",
            ...
            {
                printf("FixSpentCoins found spent unit %sSDC %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue).c_str(), pcoin->GetHash().ToString().c_str(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
...
void CWallet::ExportPeercoinKeys(int &nExportedCount, int &nErrorCount)
{
    ...
    BOOST_FOREACH(const PAIRTYPE(CBitcoinAddress, string)& item, mapAddressBook)
    {
        ...
        if (address.IsScript())
        {
            ...
            params.push_back("Stardrillcoin");
