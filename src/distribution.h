...
{
protected:
    CBitcoinAddress addrStardrillcoin;
    int64 nBalance;
...
public:
    Distribution(CBitcoinAddress addrStardrillcoin, int64 nBalance, double dDividendAmount)
        : addrStardrillcoin(addrStardrillcoin), nBalance(nBalance), addrPeercoin(addrStardrillcoin), dDividendAmount(dDividendAmount)
    {
    }

    const CBitcoinAddress GetPeershareAddress() const
    {
        return addrStardrillcoin;
    }
